...
import './css/checkout.css'; // Make sure this CSS file is styled appropriately

const BasketProcess = ({ basket }) => {
    const savedBasket = basket || JSON.parse(localStorage.getItem('basket') || '[]');
    const navigate = useNavigate(); // Create the navigate function using the hook
    const products = [
    ...
    return (
        <div className="basket-process-container">
            ...
            <div className="order-summary">
                <h2>Together</h2>
                <p>Intermediate sum: €{total}</p>
                <p>Delivery: €0.00</p>
                <p>Total (VAT included): €{total}</p>
                <button className="checkout-button" onClick={handleCheckout}>GO TO THE CHECKOUT</button>
            </div>
            </div>
        </div>
    );
